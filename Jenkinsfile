pipeline {
  ...
  environment {
    ...
    EXT_USER = 'CouchPotato'
    EXT_REPO = 'CouchPotatoServer'
    BUILD_VERSION_ARG = 'COUCHPOTATO_RELEASE'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-couchpotato'
    CONTAINER_NAME = 'couchpotato'
    DOCKERHUB_IMAGE = 'linuxserver/couchpotato'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/couchpotato'
    PR_DOCKERHUB_IMAGE = 'lspipepr/couchpotato'
    DIST_IMAGE = 'alpine'
    MULTIARCH='true'
    CI='true'
    CI_WEB='true'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              set -e
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/couchpotato") | .uuid')
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/couchpotato",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-couchpotato",\
                       "backendUrl":"https://ghcr.io/linuxserver/couchpotato",\
                       "publicUrl":"https://lscr.io/linuxserver/couchpotato"}' || :
              else
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker build \
          ...
          --label \"org.opencontainers.image.url=https://github.com/linuxserver/docker-couchpotato/packages\" \
          --label \"org.opencontainers.image.documentation=https://docs.linuxserver.io/images/docker-couchpotato\" \
          --label \"org.opencontainers.image.source=https://github.com/linuxserver/docker-couchpotato\" \
          --label \"org.opencontainers.image.version=${EXT_RELEASE_CLEAN}-ls${LS_TAG_NUMBER}\" \
          --label \"org.opencontainers.image.revision=${COMMIT_SHA}\" \
          --label \"org.opencontainers.image.vendor=linuxserver.io\" \
          --label \"org.opencontainers.image.licenses=GPL-3.0-only\" \
          --label \"org.opencontainers.image.ref.name=${COMMIT_SHA}\" \
          --label \"org.opencontainers.image.title=Couchpotato\" \
          --label \"org.opencontainers.image.description=[Couchpotato](https://couchpota.to/) is an automatic NZB and torrent downloader. You can keep a `movies I want` list and it will search for NZBs/torrents of these movies every X hours. Once a movie is found, it will send it to SABnzbd or download the torrent to a specified directory.\" \
